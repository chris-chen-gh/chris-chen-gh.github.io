...
`,componentJoin=(e,t)=>e.reduce((o,n,r)=>(r&&o.push({...t,key:r}),o.push(n),o),[]),KeycodeMap=getKeycodes().flatMap(e=>e.keycodes).reduce((e,t)=>({...e,[t.code]:t}),{}),optimizeKeycodeSequence=e=>pipeline(e,convertCharacterTaps,trimLastWait,mergeConsecutiveWaits,foldKeydownKeyupKeys,convertToCharacterStreams),cleanKeycodeSequence=e=>pipeline(e,mergeConsecutiveWaits),MacroRecorder=({selectedMacro:e,setUnsavedMacro:t,saveMacro:o,undoMacro:n,isDelaySupported:r})=>{const[s,a]=reactExports.useState(!0),[i,c]=reactExports.useState(!1),[l,d]=reactExports.useState(!1),[u,C]=reactExports.useState(!!document.fullscreenElement),{smartOptimizeEnabled:_,recordDelaysEnabled:h}=useAppSelector(getMacroEditorSettings),p=useDispatch(),[K,y]=useKeycodeRecorder(i,h),g=reactExports.useRef(null),m=reactExports.useCallback(async R=>{c(R),R?(await navigator.keyboard.lock(),y([]),a(!1),d(!0)):(navigator.keyboard.unlock(),_&&y(optimizeKeycodeSequence(K)),d(!1))},[K,c]),S=reactExports.useCallback(()=>{o(""),a(!0),d(!1)},[y,o]),b=reactExports.useCallback(()=>{n(),y([]),a(!0),d(!1)},[n]);reactExports.useEffect(()=>{a(!0),d(!1),y([])},[e]);const A=()=>[...s?e??[]:K],x=reactExports.useMemo(()=>{let R,w=A();return s||!l||!_?R=w:R=optimizeKeycodeSequence(w),R},[K,s,_,l,e]);reactExports.useEffect(()=>{x&&t(sequenceToExpression(x))},[x]);const L=reactExports.useCallback(()=>{s&&a(!1)},[s]),v=reactExports.useCallback(R=>{const w=A();w.splice(R,1),y(cleanKeycodeSequence(w)),L()},[x,e,K,s]),P=reactExports.useCallback((R,w)=>{const T=A();T.splice(R,1,[RawKeycodeSequenceAction.Delay,w]),y(cleanKeycodeSequence(T)),L()},[x,e,K,s]),B=reactExports.useMemo(()=>componentJoin(x.map(([R,w],T)=>{const O=getSequenceItemComponent(R);return jsx(Deletable,{index:T,deleteItem:v,disabled:i,children:RawKeycodeSequenceAction.Delay!==R?jsx(O,{children:R===RawKeycodeSequenceAction.CharacterStream?componentJoin(String(w).split(" ").map((M,I)=>jsx("span",{children:M},I)),jsx("span",{style:{fontFamily:"fantasy, cursive, monospace"},children:"␣"})):Array.isArray(w)?w.map(M=>getSequenceLabel(KeycodeMap[M])??M).join(" + "):getSequenceLabel(KeycodeMap[w])}):jsx(WaitInput,{index:T,value:Number(w),updateValue:P})},`${T}-${R}`)}),jsx(SequenceLabelSeparator,{})),[x]);reactExports.useEffect(()=>{const R=()=>{C(!!document.fullscreenElement)};return document.documentElement.addEventListener("fullscreenchange",R),()=>{m(!1),document.documentElement.removeEventListener("fullscreenchange",R)}},[C]);const j=reactExports.useCallback(()=>{document.fullscreenElement?document.exitFullscreen&&(m(!1),document.exitFullscreen()):document.documentElement.requestFullscreen()},[m]);return jsxs(Fragment,{children:[jsx(MacroSequenceContainer,{ref:g,$isModified:!s,children:B.length?B:jsx(NoMacroRecorded,{children:"No macro recorded yet..."})}),jsx("div",{style:{border:"none",maxWidth:960,width:"100%",display:"flex",justifyContent:"center",transform:"translate(-0px, -21px)"},children:jsx(MacroEditControls,{isFullscreen:u,isEmpty:!e||!e.length,optimizeRecording:_,recordDelays:h,isRecording:i,addText:()=>{},deleteMacro:S,toggleOptimizeRecording:()=>{p(setMacroEditorSettings({smartOptimizeEnabled:!_}))},toggleRecordDelays:()=>{p(setMacroEditorSettings({recordDelaysEnabled:!h}))},toggleFullscreen:j,undoChanges:b,saveChanges:()=>o(),hasUnsavedChanges:!s,recordingToggleChange:m,isDelaySupported:r})})]})},TextArea=styled.textarea`
  box-sizing: border-box;
  background: var(--bg_control);
  padding: 5px 10px;
