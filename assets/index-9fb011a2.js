...
`,KeycodeContent=styled.div`
  text-overflow: ellipsis;
  overflow: hidden;
`,MacroContent=styled.div`
  width: 40px;
  height: 40px;
  position: relative;
  background-image: url(${e=>e.image_path});
...
`,componentJoin=(e,t)=>e.reduce((o,n,r)=>(r&&o.push({...t,key:r}),o.push(n),o),[]),KeycodeMap=getKeycodes().flatMap(e=>e.keycodes).reduce((e,t)=>({...e,[t.code]:t}),{}),optimizeKeycodeSequence=e=>pipeline(e,convertCharacterTaps,trimLastWait,mergeConsecutiveWaits,foldKeydownKeyupKeys,convertToCharacterStreams),cleanKeycodeSequence=e=>pipeline(e,mergeConsecutiveWaits),MacroRecorder=({selectedMacro:e,setUnsavedMacro:t,saveMacro:o,undoMacro:n,isDelaySupported:r})=>{const[s,a]=reactExports.useState(!0),[i,c]=reactExports.useState(!1),[l,d]=reactExports.useState(!1),[u,C]=reactExports.useState(!!document.fullscreenElement),{smartOptimizeEnabled:_,recordDelaysEnabled:h}=useAppSelector(getMacroEditorSettings),p=useDispatch(),[K,y]=useKeycodeRecorder(i,h),g=reactExports.useRef(null),m=reactExports.useCallback(async w=>{c(w),w?(await navigator.keyboard.lock(),y([]),a(!1),d(!0)):(navigator.keyboard.unlock(),_&&y(optimizeKeycodeSequence(K)),d(!1))},[K,c]),S=reactExports.useCallback(()=>{o(""),a(!0),d(!1)},[y,o]),b=reactExports.useCallback(()=>{n(),y([]),a(!0),d(!1)},[n]);reactExports.useEffect(()=>{a(!0),d(!1),y([])},[e]);const L=()=>[...s?e??[]:K],M=reactExports.useMemo(()=>{let w,f=L();return s||!l||!_?w=f:w=optimizeKeycodeSequence(f),w},[K,s,_,l,e]);reactExports.useEffect(()=>{M&&t(sequenceToExpression(M))},[M]);const R=reactExports.useCallback(()=>{s&&a(!1)},[s]),v=reactExports.useCallback(w=>{const f=L();f.splice(w,1),y(cleanKeycodeSequence(f)),R()},[M,e,K,s]),I=reactExports.useCallback((w,f)=>{const O=L();O.splice(w,1,[RawKeycodeSequenceAction.Delay,f]),y(cleanKeycodeSequence(O)),R()},[M,e,K,s]),P=reactExports.useMemo(()=>componentJoin(M.map(([w,f],O)=>{const T=getSequenceItemComponent(w);return jsx(Deletable,{index:O,deleteItem:v,disabled:i,children:RawKeycodeSequenceAction.Delay!==w?jsx(T,{children:w===RawKeycodeSequenceAction.CharacterStream?componentJoin(String(f).split(" ").map((A,j)=>jsx("span",{children:A},j)),jsx("span",{style:{fontFamily:"fantasy, cursive, monospace"},children:"␣"})):Array.isArray(f)?f.map(A=>getSequenceLabel(KeycodeMap[A])??A).join(" + "):getSequenceLabel(KeycodeMap[f])}):jsx(WaitInput,{index:O,value:Number(f),updateValue:I})},`${O}-${w}`)}),jsx(SequenceLabelSeparator,{})),[M]);reactExports.useEffect(()=>{const w=()=>{C(!!document.fullscreenElement)};return document.documentElement.addEventListener("fullscreenchange",w),()=>{m(!1),document.documentElement.removeEventListener("fullscreenchange",w)}},[C]);const B=reactExports.useCallback(()=>{document.fullscreenElement?document.exitFullscreen&&(m(!1),document.exitFullscreen()):document.documentElement.requestFullscreen()},[m]);return jsxs(Fragment,{children:[jsx(MacroSequenceContainer,{ref:g,$isModified:!s,children:P.length?P:jsx(NoMacroRecorded,{children:"No macro recorded yet..."})}),jsx("div",{style:{border:"none",maxWidth:960,width:"100%",display:"flex",justifyContent:"center",transform:"translate(-0px, -21px)"},children:jsx(MacroEditControls,{isFullscreen:u,isEmpty:!e||!e.length,optimizeRecording:_,recordDelays:h,isRecording:i,addText:()=>{},deleteMacro:S,toggleOptimizeRecording:()=>{p(setMacroEditorSettings({smartOptimizeEnabled:!_}))},toggleRecordDelays:()=>{p(setMacroEditorSettings({recordDelaysEnabled:!h}))},toggleFullscreen:B,undoChanges:b,saveChanges:()=>o(),hasUnsavedChanges:!s,recordingToggleChange:m,isDelaySupported:r})})]})},TextArea=styled.textarea`
  box-sizing: border-box;
  background: var(--bg_control);
  padding: 5px 10px;
...
`,CanvasImageContainer=styled.div`
  box-shadow: inset -1px -1px 0 rgb(0 0 0 / 20%),
    inset 1px 1px 0 rgb(255 255 255 / 10%);
  background-image: url(${e=>e.image_path});
